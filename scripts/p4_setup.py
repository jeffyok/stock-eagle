...
conn = pymysql.connect(host='127.0.0.1', port=3306, user='root', passwd='123456', charset='utf8mb4')
cur = conn.cursor()

# 确保数据库存在
cur.execute("CREATE DATABASE IF NOT EXISTS db_stockeagle CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci")
cur.execute("USE db_stockeagle")
...
for r in rules:
    cur.execute("""
        INSERT INTO t_risk_rule (rule_key, rule_name, rule_value, rule_type, description)
        VALUES (%s, %s, %s, %s, %s)
        ON DUPLICATE KEY UPDATE rule_name=VALUES(rule_name)
    """, r)
print("[OK] t_risk_rule initial data")
...
cur.close()
conn.close()
print("\n[P4] All tables created successfully!")
